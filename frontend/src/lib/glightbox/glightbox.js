...
export class GlightboxInit {
	...
	showSlide(index = 0, first = false) {
		...
		this.slideDescriptionContained =
			this.slideDescription && _.hasClass(this.slideDescription.parentNode, "gslide-media");
	...
	slideAnimateIn(slide, first) {
		...
		let prevData = {
			...
			slideConfig: _.isNil(this.prevActiveSlideIndex)
				? null
				: this.elements[this.prevActiveSlideIndex].slideConfig,
			trigger: _.isNil(this.prevActiveSlideIndex)
				? null
	...
	getElements() {
		...
		if (
			!_.isNil(this.settings.elements) &&
			_.isArray(this.settings.elements) &&
			...
			this.events["outClose"] = _.addEvent("click", {
				...
				withCallback: (e, target) => {
					if (
						...
						if (
							!_.closest(e.target, ".gbtn") &&
							!_.hasClass(e.target, "gnext") &&
							!_.hasClass(e.target, "gprev")
						) {
							this.close();
						}
					}
	...
	resize(slide = null) {
		...
		if (
			description &&
			(_.hasClass(description, "description-bottom") ||
				_.hasClass(description, "description-top")) &&
			!_.hasClass(description, "gabsolute")
		) {
		...
		if (video) {
			let ratio = _.has(this.settings.plyr.config, "ratio")
