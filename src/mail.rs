...
impl Handler<SignupMessage> for MailExecutor {
    ...
    fn handle(&mut self, msg: SignupMessage, _: &mut Self::Context)
        -> Self::Result {
        let subject = format!("{}", Subject { member: &msg.member }).trim()
            .to_string();
        let body = format!("{}", Body { member: &msg.member }).trim()
            .to_string();

        let email = EmailBuilder::new()
            .to((msg.member.eltern_mail.clone(), msg.member.eltern_name.clone()))
            .from((self.config.email_username.clone(),
        ...
        let mut mailer = SmtpTransport::simple_builder(
            ...
            .credentials(Credentials::new(self.config.email_username.clone(),
